class ClubActivity < Activity
  ...
  def brag_to_channel!
    ...
    if team.channel_threads_for(channel_id) == 'activity'
      summary_with_channel = to_slack_summary(channel_id).merge(channel: channel_id, as_user: true)
      ...
      if channel_message
        ...
        details = to_slack_details(channel_id)
        if details[:blocks].any?
      ...
      message_with_channel = to_slack(channel_id).merge(channel: channel_id, as_user: true)
      ...
      channel_message = { ts: channel_message['ts'], channel: channel_id } if channel_message
    end
    channel_message
  end
