      ...
      context 'with a connected user' do
        ...
        context 'with an expired subscription' do
          ...
          it 'does not sync user' do
            ...
            expect_any_instance_of(User).not_to receive(:sync_and_brag!)
            post '/api/strava/event',
                 JSON.dump(
                   event_data.merge(
                     aspect_type: 'create',
        ...
        context 'with an existing activity' do
          ...
          it 'ignores unknown aspects' do
            ...
            expect_any_instance_of(User).not_to receive(:sync_and_brag!)
            expect_any_instance_of(User).not_to receive(:rebrag!)
            expect_any_instance_of(User).not_to receive(:unbrag!)
            post '/api/strava/event',
                 JSON.dump(
                   event_data.merge(
                     aspect_type: 'unknown',
